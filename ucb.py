...
def getHexBlock(ciphertext, i, blockSize):
    block = ciphertext.encode('hex')[(i*blockSize)*2:
(i+1)*blockSize*2]
    #print "------"
    #print block
    #print (i*blockSize)*2
...
A="A"
# Discover block size by feeding ECB oracle increasing size inputs to
...
# Need to use 3 blocks of A since we don't know how long the prefix
string is
...
blocks = splitIntoBlocks(checkpoint2_oracle(A*(3*blockSize),
sock).encode('hex'), blockSize*2)
...
# repeatedly increase the starting A's in the third block until you
get the two repeated ciphertext blocks
# that were used to detect ECB mode
# the moment that they are equal, that means that the Z is pushed out
into the next block
...
numberOfUnknownBlocks = len(checkpoint2_oracle("", sock))/blockSize -
x
# Now we can proceed with the previous algorithm, just adding A*offset
before our strings
# and adding x to the j block offset
crack={}
decryptedMessage = ""
# Decrypt each block, one at a time
# Added one to the numberof unknown blocks since if the offset is
large, it doesn't decrypt the final block

for j in range(numberOfUnknownBlocks+1):
    # j is the block I need to keep
    # Decrypt the block iteratively, since you can
    for i in range(blockSize):
        addToDecryptionDictionary(A*offset + A*(blockSize-i-
1)+decryptedMessage, crack, j+x, blockSize, sock)
        block = getHexBlock(checkpoint2_oracle(A*offset +
A*(blockSize-i-1), sock), j+x, blockSize)

        # at the very end, after the last byte,
        # the padding changes since the size of the message is
changing
